...
static edict_t *SelectRandomDeathmatchSpawnPoint( edict_t *ent )
{
	...
	if( ent && GS_TeamBasedGametype( &g_gs ) )
...
bool G_OffsetSpawnPoint( vec3_t origin, vec3_t box_mins, vec3_t box_maxs, float radius, bool checkground )
{
	...
	for( i = 0; i < ( rows * columns ); i++ )
	{
		...
		if( trace.startsolid || trace.allsolid || trace.ent != -1 )
		{
			if( trace.ent == 0 )
				worldfound++;
			else if( trace.ent < g_gs.maxclients )
				playersFound++;
			continue;
		}
...
void G_SpawnQueue_ReleaseTeamQueue( int team )
{
	...
	// try to spawn them
	for( count = 0; ( queue->start < queue->head ) && ( count < g_gs.maxclients ); queue->start++, count++ )
	{
		if( queue->list[queue->start % MAX_CLIENTS] <= 0 || queue->list[queue->start % MAX_CLIENTS] > g_gs.maxclients )
			continue;
...
void G_SpawnQueue_AddClient( edict_t *ent )
{
	...
	if( ENTNUM( ent ) <= 0 || ENTNUM( ent ) > g_gs.maxclients )
		return;
...
void G_SpawnQueue_Think( void )
{
	...
	for( team = TEAM_SPECTATOR; team < GS_MAX_TEAMS; team++ )
	{
		...
		case SPAWNSYSTEM_WAVES:
			...
			{
				maxCount = ( queue->wave_maxcount < 1 ) ? g_gs.maxclients : queue->wave_maxcount; // max count per reinforcement wave
